...
public class DFSStarterTimeOut {
    ...
    static ArrayList<Integer> findCycle(Graph graph, DFSInfo info) {
       ...
       for (int i=1; i<graph.n;i++){
          ...
          while(node!=null){
            ...
            System.out.println("\tv:"+node.data);
             ...
             System.out.println(info.d[i]);
             System.out.println(info.f[i]);
             */
             if (info.f[i]<=info.f[node.data]){
                System.out.println("Find backward Edge!");
                ArrayList<Integer> cycle = new ArrayList<Integer>();
                Integer start = node.data;
                Integer end = i;
                Integer parent = info.parent[end];
                cycle.add(end);
                while (parent!=start){
    ...
    public static void main(String[] args) throws IOException {
       ...
       info.printDFSInfo();
       ArrayList<Integer> cycle = findCycle(graph, info);
...
Item head = new Item(node.data);
head = new Item(parent, head);
Item head = new Item(node.data, null);
