...
def lookup(p, h, key):
    ...
    if key <= p.child0.guide:
        if (lookup(p.child0, h-1, key)) != -1:
            return p.value+lookup(p.child0, h-1, key)
        else:
            return -1
    elif key <= p.child1.guide:
        if (lookup(p.child1, h-1, key)) != -1:
            return p.value+lookup(p.child1, h-1, key)
        else:
            return -1
    elif p.child2 != None and key <= p.child2.guide:
        if (lookup(p.child2, h-1, key)) != -1:
...
def main():
    ...
    with fileinput.FileInput("test7.in") as input:
